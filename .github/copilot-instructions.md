...
This repository contains technical talks, presentations, and accompanying demo projects. Follow
these instructions when helping prepare new talks, presentations, or demos.
...
When new categories are needed (e.g., `conference/`, `workshop/`, `meetup/`), follow the same nested
structure.
...
> ⚠️ Always use **bun** / **bunx** (not npm/npx/yarn/pnpm) for managing JavaScript dependencies and
...
All presentations are written in **Markdown** for [Slidev](https://sli.dev), a developer-friendly,
Markdown-based presentation framework.
...
- **Use mental models and analogies** to make abstract concepts concrete (e.g., "Think of middleware
...
- **Use Mermaid diagrams** (` ```mermaid `) for architecture, flow, and sequence visualization —
  Slidev renders them natively. **Do not use ASCII box-drawing diagrams** (they break in Slidev's
  code font rendering)
- **Include "Common Pitfalls" and "Best Practices" sections** — audiences love practical do's and
  don'ts
...
- **Always format Markdown files** before committing — run `bun run format` (Prettier) to ensure
  consistent formatting. The VS Code workspace is configured to format on save automatically.

---

## Demo Projects

Demo projects live under `<Talk Name>/demo/` and are meant for **live coding during the
presentation**.
...
- **No authentication to external services** — use in-memory data, hardcoded values, or local
  resources
...
- **Add XML doc comments** (`/// <summary>`) on public classes — include **what the class does** and
  **what concept it demonstrates** (e.g., "Demonstrates: short-circuiting the pipeline")
- **Add inline comments** only to explain _why_, not _what_ — focus on teaching moments relevant to
  the talk
- **Organize related code in feature folders** (e.g., `Middleware/` folder for all middleware
  classes)
...
- Use comments to **label the "before" and "after"** sections (e.g.,
...
2. **Create the folder structure**:
   ...
   <category>/<Talk Name>/
     ...
     demo/
       <ProjectName>/
   ```
3. **Write a `README.md`** for the talk with a brief title, description, topic list, and
   instructions to run the demo
4. **Write `presentation.md`** with Slidev YAML frontmatter at the top, following the structure and
   style above
...
8. **Update `package.json`** — add `slides:<name>`, `build:<name>`, and `export:<name>` scripts for
   the new talk, then run `bun install`
...
The root `README.md` should list all talks with links. When adding a new talk, add it under the
...
- **Commits must be atomic** — each commit should contain exactly one logical change (e.g., don't
  mix a new demo with a README update in the same commit)
...
- **Scope is optional** but encouraged — use the talk name or category (e.g.,
  `feat(middlewares): add ordering demo`)
...
If a talk covers a different technology (TypeScript, Python, Go, etc.), adapt the demo conventions
accordingly:
